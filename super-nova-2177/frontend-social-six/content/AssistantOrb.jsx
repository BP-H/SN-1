...
import { RiVoiceAiFill, RiEdit2Fill } from "react-icons/ri";
...
export default function AssistantOrb() {
  ...
  const menuStyle =
    window.innerWidth < 1024
      ? { left: pos.x - 170, top: pos.y - 70 }
      : { right: 130, top: pos.y };

  return (
    <div
      className="fixed flex z-999999"
      style={{
        left: pos.x,
        top: pos.y,
        position: "fixed",
    ...
    >
      ...
      {menuOpen && (
        <div
          ...
          style={menuStyle}
        >
          <button onClick={() => simulateSpeak("Opening chat...")} className={btnClass}>
            <IoChatbox />
          </button>
          <button onClick={() => simulateSpeak("Reacting to this post...")} className={btnClass}>
            <AiFillLike />
          </button>
          <button onClick={() => simulateSpeak("Commenting on this post...")} className={btnClass}>
            <RiEdit2Fill />
          </button>
        </div>
      )}
      ...
      {popup && (
        <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-85 text-white px-5 py-3 rounded-[20px] text-sm max-w-[80%] text-center z-[2000]">
          {popup}
        </div>
      )}
    </div>
