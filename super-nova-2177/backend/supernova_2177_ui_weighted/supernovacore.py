...
@router.get("/universe/info", tags=["System"])
...
# --- Garantir que SessionLocal está definido no escopo do módulo ---
if 'SessionLocal' not in globals():
    try:
        from db_models import SessionLocal
    except ImportError:
        # Fallback se db_models não estiver disponível
...
def get_music_generator(
    ...
    return MusicGeneratorService(db, user) # ADDED
...
@router.get("/users/me", response_model=HarmonizerOut, tags=["Harmonizers"])
def read_users_me(current_user: Harmonizer = Depends(get_current_active_user)):
    return current_user


@router.get("/users/me/influence-score", tags=["Harmonizers"])
...
@router.get("/users/{username}", response_model=HarmonizerOut, tags=["Harmonizers"])
...
@router.get("/users/{username}/followers", tags=["Harmonizers"])
...
@router.get("/users/{username}/following", tags=["Harmonizers"])
...
@router.get("/users/search", tags=["Harmonizers"])
...
@router.get("/system/entropy-details", response_model=EntropyDetails, tags=["System"])
...
@router.get("/system/collective-entropy", tags=["System"])
...
@router.get("/network-analysis/", tags=["System"])
...
@router.get("/api/epistemic-audit", tags=["System"])
def epistemic_audit():
...
@router.get("/api/global-epistemic-state", tags=["System"])
...
@router.get("/api/predict-user/{user_id}", tags=["Predictions"])
...
@router.get("/api/system-predictions", tags=["Predictions"])
...
@router.get("/api/quantum-status", tags=["System"])
...
# RFC_V5_1_INIT
@router.get("/resonance-summary", tags=["System"])
...
@router.get("/api/adaptive-config-status", tags=["System"])
...
@router.get("/api/scientific-discoveries", tags=["System"])
...
@router.get("/sim/negentropy", tags=["Simulations"], status_code=status.HTTP_200_OK)
...
def _species_shares(active: List[Species]) -> Dict[Species, float]:
    ...
    if not present:
        return {}
    return {s: SPECIES_WEIGHTS.get(s, 0.0) for s in present}
