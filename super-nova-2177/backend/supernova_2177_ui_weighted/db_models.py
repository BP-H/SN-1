...
if DB_MODE == "central":
    ...
    DATABASE_URL = os.getenv(
        "DATABASE_URL", "sqlite:///supernova_local.db"
    )
...
class Harmonizer(Base):
    ...
    cultural_preferences = Column(JSON, default=lambda: [])
...
class Comment(Base):
    ...
    proposal = relationship("Proposal", backref="comments")
...
class Proposal(Base):
    ...
    author_type = Column(String, default="human")
    ...
    author = relationship("Harmonizer", backref="proposals")
...
class ProposalVote(Base):
    ...
    harmonizer_id = Column(Integer, ForeignKey("harmonizers.id"), primary_key=True)
    vote = Column(String, nullable=False, default="neutral")
    voter_type = Column(String, default="human")
...
class Decision(Base):
    ...
    status = Column(String, default="undecided")  # accepted / rejected / undecided
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
...
class Run(Base):
    ...
    decision = relationship("Decision", backref="runs")
...
class BranchVote(Base):
    ...
    harmonizer_id = Column(ForeignKey("harmonizers.id"), nullable=False)
    vote = Column(Boolean, nullable=False)
    timestamp = Column(DateTime, default=datetime.datetime.utcnow)

    __table_args__ = (
        UniqueConstraint("branch_id", "harmonizer_id", name="unique_branch_voter"),
    )
